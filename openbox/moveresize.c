...
void moveresize_start(ObClient *c, gint x, gint y, guint b, guint32 cnr) {
	...
	if (moveresize_in_progress || !c->frame->visible ||
		!(mv ? (c->functions & OB_CLIENT_FUNC_MOVE)
	...
	if (config_resize_redraw && !moving && obt_display_extension_sync &&
		...
		moveresize_alarm =
			XSyncCreateAlarm(obt_display,
							 XSyncCACounter | XSyncCAValue | XSyncCAValueType |
...
void moveresize_end(gboolean cancel) {
	...
	*/
	client_configure(moveresize_client, (cancel ? start_cx : cur_x),
...
static void do_move(gboolean keyboard, gint keydist) {
	...
	client_configure(moveresize_client, cur_x, cur_y, cur_w, cur_h, TRUE, FALSE,
					 FALSE);
...
static void do_resize(void) {
	...
	if (!(w == moveresize_client->area.width &&
		...
		if (config_resize_redraw && obt_display_extension_sync &&
			...
			waiting_for_sync == 0 && moveresize_client->sync_request &&
		...
		client_configure(moveresize_client, cur_x, cur_y, cur_w, cur_h, TRUE,
						 FALSE, TRUE);
	}
...
static void resize_with_keys(KeySym sym, guint state) {
	...
	if (state & obt_keyboard_modkey_to_modmask(OBT_KEYBOARD_MODKEY_SHIFT)) {
		...
		ObClientDirectionalResizeType resize_type =
			key_resize_edge == dir ? CLIENT_RESIZE_GROW : CLIENT_RESIZE_SHRINK;
		...
		if (moveresize_client->size_inc.width > 1) {
			...
			resist = 1;
		} else if (state & obt_keyboard_modkey_to_modmask(
	...
	if (moveresize_client->max_horz && (key_resize_edge == OB_DIRECTION_WEST ||
										key_resize_edge == OB_DIRECTION_EAST)) {
		/* unmax horz */
		was_max_horz = TRUE;
...
gboolean moveresize_event(XEvent *e) {
	...
	if (e->type == ButtonPress) {
		...
		if (sym == XK_Escape) {
			...
			moveresize_end(FALSE);
			used = TRUE;
		} else if (sym == XK_Right || sym == XK_Left || sym == XK_Up ||
				   sym == XK_Down) {
