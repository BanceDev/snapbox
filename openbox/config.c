...
static void parse_single_per_app_settings(xmlNodePtr app,
	...
	if ((n = obt_xml_find_node(app->children, "size"))) {
		if ((c = obt_xml_find_node(n->children, "width"))) {
			if (!obt_xml_node_contains(c, "default")) {
				gchar *s = obt_xml_node_string(c);
				config_parse_relative_number(s, &settings->width_num,
		...
		if ((c = obt_xml_find_node(n->children, "height"))) {
			if (!obt_xml_node_contains(c, "default")) {
				gchar *s = obt_xml_node_string(c);
				config_parse_relative_number(s, &settings->height_num,
...
static void parse_per_app_settings(xmlNodePtr node, gpointer d) {
	...
	for (; app; app = obt_xml_find_node(app->next, "application")) {
		...
		gboolean name_set, class_set, role_set, title_set, type_set,
		...
		config_per_app_settings =
			g_slist_append(config_per_app_settings, (gpointer)settings);
	}
}
...
static void parse_mouse(xmlNodePtr node, gpointer d) {
	...
	for (n = obt_xml_find_node(node, "context"); n;
		...
		while (frame_next_context_from_string(modcxstr, &cx)) {
			...
			for (nbut = obt_xml_find_node(n->children, "mousebind"); nbut;
				...
				for (nact = obt_xml_find_node(nbut->children, "action"); nact;
					 nact = obt_xml_find_node(nact->next, "action")) {
					ObActionsAct *action;

					/* actions_parse() creates one ref to the action, but we
					 * need exactly one ref per binding we use it for. */
...
static void parse_theme(xmlNodePtr node, gpointer d) {
	...
	for (n = obt_xml_find_node(node, "font"); n;
...
static void parse_desktops(xmlNodePtr node, gpointer d) {
	...
	if ((n = obt_xml_find_node(node, "names"))) {
		...
		for (nname = obt_xml_find_node(n->children, "name"); nname;
...
static void parse_menu(xmlNodePtr node, gpointer d) {
	...
	if ((n = obt_xml_find_node(node, "showIcons"))) {
		...
		if (config_menu_show_icons)
			g_message(_("Openbox was compiled without image loading support. "
						"Icons in menus will not be loaded."));
#endif
	}

	for (node = obt_xml_find_node(node, "file"); node;
		 node = obt_xml_find_node(node->next, "file")) {
		gchar *c = obt_xml_node_string(node);
		config_menu_files =
			g_slist_append(config_menu_files, obt_paths_expand_tilde(c));
		g_free(c);
	}
}
...
static void bind_default_keyboard(void) {
	ObDefKeyBind *it;
	ObDefKeyBind binds[] = {{"A-Tab", "NextWindow"},
...
static void bind_default_mouse(void) {
	...
	for (it = binds; it->button; ++it)
		mouse_bind(it->button, frame_context_from_string(it->context), it->mact,
